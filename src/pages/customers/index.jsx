...
const exportToPDF = () => {
  if (!customers.length) return toast.error("Hakuna data ya ku-export");
  ...
  doc.setFontSize(18);
  doc.text(`${user?.office_name || "Office"} - Customer Summary`, 14, 14);
  ...
  // Top summary cards
  drawCard(15, cardY, cardW, cardH, "Jumla ya Wateja", `${totals.totalCustomers}`, [219, 234, 254]);
  drawCard(20 + cardW, cardY, cardW, cardH, "Walioongezwa Leo", `${totals.newToday}`, [220, 252, 231]);
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16);
  doc.text(`${user?.office_name || "Office"} - Customers Report`, 14, 12);
  ...
  doc.save(`customers_${dayjs().format("YYYYMMDD_HHmm")}.pdf`);
};
