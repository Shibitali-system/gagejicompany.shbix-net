...
const MeetingNew = () => {
  ...
  const [attendeePositions, setAttendeePositions] = useState({});
  ...
  const addImplementationPoint = () => setImplementationPoints(prev => [...prev, { description: "", responsible_id: "" }]);
  const removeImplementationPoint = idx => setImplementationPoints(prev => prev.filter((_, i) => i !== idx));
  const updateImplementationPoint = (idx, field, value) => setImplementationPoints(prev => prev.map((p, i) => i === idx ? { ...p, [field]: value } : p));
  ...
  const handlePositionChange = (id, position) => setAttendeePositions(prev => ({ ...prev, [id]: position }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Insert attendees with positions
      if (attendees.length > 0) {
        const attendeeRecords = attendees
          ...
          .map(aid => ({
            ...
            user_id: aid,
            position: attendeePositions[aid] || ""
          }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <h1 className="text-3xl font-bold text-[#2563EB]">Create New Meeting</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card title="Virtual Meeting">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={isVirtual} onChange={e => setIsVirtual(e.target.checked)} />
              <span className="text-sm">Is this a virtual meeting?</span>
            </label>
          </Card>

          <Card title="Tags (comma separated)">
            <input type="text" value={tags} onChange={e => setTags(e.target.value)}
              placeholder="sales, board, planning"
              className="w-full border border-[#e5e7eb] rounded-xl px-4 py-2" />
          </Card>
        </div>

        {/* Implementation Points */}
        <Card title="Implementation Points">
          {implementationPoints.map((point, idx) => (
            <div key={idx} className="flex gap-2 items-center mb-2">
              <input type="text" placeholder="Implementation description" value={point.description}
              ...
              <select value={point.responsible_id}
              ...
              <button onClick={() => removeImplementationPoint(idx)}
                className="text-red-500 p-1 rounded hover:bg-red-100"><FaTrash /></button>
            </div>
          ))}
          <button onClick={addImplementationPoint}
        ...
        <Card title="Attendees">
          <input type="text" placeholder="Search attendee..." value={searchAttendee}
          ...
          <div className="max-h-60 overflow-y-auto">
            {filteredUsers.map(u => (
              <div key={u.id} className="flex items-center gap-2 mb-1">
                <input type="checkbox" checked={attendees.includes(u.id)}
                  onChange={e => handleAttendeeCheck(u.id, e.target.checked)} />
                <span className="flex-1">{u.name}</span>
                {attendees.includes(u.id) && (
                  <input type="text" placeholder="Position (e.g., Chairman)"
                    value={attendeePositions[u.id] || ""}
        ...
        <div className="flex justify-end gap-3">
          <button onClick={() => navigate(-1)}
            className="px-5 py-2 rounded-xl border border-[#e5e7eb] hover:bg-gray-100">Cancel</button>
          ...
          <button onClick={handleSave} disabled={saving}
            ...
            <FaSave /> {saving ? "Saving..." : "Create Meeting"}
          </button>
        </div>
