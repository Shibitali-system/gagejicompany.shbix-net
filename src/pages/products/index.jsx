...
const handleExportPDF = () => {
  if (!filteredProducts.length) return toast.error("Hakuna bidhaa za ku-export");
  ...
  doc.setFontSize(18);
  doc.text(`${sellerInfo?.office_name || "Office"} - Products Summary`, 14, 14);

  doc.setFontSize(10);
  doc.text(`Generated: ${new Date().toLocaleString()}`, pageWidth - 75, 14);
  ...
  // Top summary cards
  drawCard(15, cardY, cardW, cardH, "Unique Products", `${totals.uniqueProducts}`, [219, 234, 254]);
  drawCard(20 + cardW, cardY, cardW, cardH, "Total Stock", `${totals.totalQuantity}`, [220, 252, 231]);
  drawCard(25 + cardW * 2, cardY, cardW, cardH, "Total Purchase Value", `${formatTZS(totals.totalPurchase)}`, [254, 249, 195]);

  cardY += 30;
  drawCard(15, cardY, cardW, cardH, "Total Sales Value", `${formatTZS(totals.totalSales)}`, [237, 233, 254]);
  drawCard(20 + cardW, cardY, cardW, cardH, "Expected Profit", `${formatTZS(totals.totalProfit)}`, [254, 226, 226]);
  drawCard(25 + cardW * 2, cardY, cardW, cardH, "Expired Products", `${totals.expiredCount}`, [219, 234, 254]);
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16);
  doc.text(`${sellerInfo?.office_name || "Office"} - Products Report`, 14, 12);
  doc.setFontSize(9);
  doc.text(`Generated: ${new Date().toLocaleString()}`, pageWidth - 70, 12);
  ...
  const tableRows = filteredProducts.map((p, index) => {
  ...
  doc.autoTable({
    ...
    didParseCell: function (data) {
      if (data.column.index === 4 || data.column.index === 5) data.cell.styles.textColor = [17, 24, 39]; // prices
      if (data.column.index === 6 && data.section === "body") {
        data.cell.styles.textColor = [220, 38, 38]; // stock highlight if needed
      }
      if (data.column.index === 9 && data.section === "body") data.cell.styles.textColor = [37, 99, 235]; // profit
    }
  });

  doc.save(`products_${new Date().toISOString().replace(/[:.-]/g, "")}.pdf`);
};
...
const handleExportProductStatement = async (productId) => {
  ...
  if (product.stock !== null) {
    ...
    const initialStock = product.stock - totalIn + totalOut;
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16);
  doc.text(`${sellerInfo?.office_name || "Office"} - Product Statement`, 14, 14);
  ...
  const drawCard = (x, y, w, h, title, value, bgColor) => {
    ...
    doc.setTextColor(17, 24, 39);
    doc.setFontSize(14);
    doc.text(value.toString(), x + 3, y + 16);
  };
  ...
  drawCard(15, cardY, cardW, cardH, "Product", product.name, [219, 234, 254]);
  drawCard(20 + cardW, cardY, cardW, cardH, "Current Stock", balance, [220, 252, 231]);
  drawCard(25 + cardW * 2, cardY, cardW, cardH, "Total Movements", movements.length, [254, 249, 195]);
  ...
  doc.autoTable({
    ...
    didParseCell: function (data) {
      // highlight In/Out/Balances
      if (data.column.index === 1 && data.section === "body") data.cell.styles.textColor = [22, 163, 74]; // green in
      if (data.column.index === 2 && data.section === "body") data.cell.styles.textColor = [220, 38, 38]; // red out
      if (data.column.index === 3 && data.section === "body") data.cell.styles.textColor = [37, 99, 235]; // balance blue
      if (data.row.index === 0) data.cell.styles.fontStyle = "bold"; // initial stock bold
    },
  });
